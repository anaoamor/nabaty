<?php
  ...
  //halaman tidak bisa diakses langsung, harus ada query string id_diskon
  if(empty(Input::get('id_diskon'))){
    die('Maaf halaman ini tidak bisa diakses langsung');
  }

  $diskon = new Diskon();
  $diskon->generate(Input::get('id_diskon'));

  $bibit = new BibitTanaman();
  $bibit->generate($diskon->getItem('id_bibit_tanaman'));
  
  if(!empty($_POST)){
    $pesanErrorDiskon = $diskon->validasi($_POST);
    if(empty($pesanErrorDiskon)){
      $diskon->update($diskon->getItem('id_diskon'));
      ...
      <div class="container">
        <div class="row justify-content-center">
          <div class="col-auto py-4">
            <h1 class="h4 me-auto">Edit Diskon</h1>
          </div>
        </div>
      </div>
        ...
        <div class="col-8">
          ...
          <form method="post">
            <div class="mb-3 form-group row">
              <label class="col-sm-3 col-form-label">ID</label>
              <div class="col-sm-9">
                <label class="col-form-label"><?php echo $diskon->getItem('id_diskon'); ?></label>
              </div>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
              ...
              <div class="input-group justify-content-between">
                ...
                value="<?php $waktuMulai = new DateTime($diskon->getItem('waktu_muali')); echo $waktuMulai->format('m/d/Y'); ?>">
                <span> s.d </span>
                <input id="waktu_akhir" name="waktu_akhir" width="276" placeholder="Akhir" 
                value="<?php $waktuAkhir = new DateTime($diskon->getItem('waktu_akhir')); echo $waktuAkhir->format('m/d/Y'); ?>">
                <?php
    ...
    <script>
      ...
      $(document).ready(function(){
        $('#btn-cancel').click(function(){
          var konfirmasi = window.confirm("Batal memperbarui diskon?");
          if(konfirmasi){
            window.location = 'diskon.php';
          }
        });
      });
    </script>
